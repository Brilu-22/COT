        ...
        //  if(!$flag_retweeted) {
		    ...
		    if ($comment != '') {
			 ...
			 foreach ($comments as $comment) {
				 ...
				 // Check if $tweet_user is valid
				 if ($tweet_user !== false) {
					 echo '<div class="comment">';
					 echo '<strong>' . htmlspecialchars($tweet_user->name) . '</strong> @' . htmlspecialchars($tweet_user->username) . '<br>';
					 echo '<p>' . htmlspecialchars($comment->comment) . '</p>';
					 
					 // Check if $tweet_user->img is not null
					 if ($tweet_user->img != null) {
						 echo '<img src="' . $tweet_user->img . '" alt="" class="img-user-tweet" />';
					 }
					 
					 echo '</div>';
				 } else {
					 // Handle the error if user data is not found
					 error_log("No user found for user_id: " . $comment->user_id);
					 // Optionally, you can skip this comment or continue to the next iteration
					 continue;
				 }
			 }
			}
